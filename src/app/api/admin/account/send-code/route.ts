  ...
  try {
    ...
    if (changeCount < 1) {
      return NextResponse.json(
        {
          error: "İlk e-posta veya şifre değişikliğiniz doğrulama kodu gerektirmez. Doğrudan kaydedebilirsiniz.",
        },
        { status: 400 },
      );
    }
    ...
    if (!looksLikeEmail(u.emailOrPhone)) {
      return NextResponse.json(
        { error: "Doğrulama kodu için kayıtlı geçerli bir e-posta adresi olmalıdır." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({ success: true, sentTo: masked, expiresInMinutes: 15 });
  } catch (e) {
