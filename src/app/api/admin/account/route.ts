...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const nextLogin =
      typeof raw.emailOrPhone === "string" ? normalizeLogin(raw.emailOrPhone) : curr.emailOrPhone;
    ...
    if (emailChanging) {
      const other = await d.db.select().from(users).where(eq(users.emailOrPhone, nextLogin)).limit(1);
      if (other[0] && other[0].id !== session.id) {
        return NextResponse.json({ error: "Bu oturum adı başka bir hesapta kullanılıyor." }, { status: 409 });
      }
    }
    ...
    if (credentialChange && changeCount >= 1) {
      ...
      if (!looksLikeEmail(curr.emailOrPhone)) {
        return NextResponse.json(
          {
            error:
              "Doğrulama kodu için hesabınızda kayıtlı geçerli bir e-posta olmalıdır. İlk değişiklikte e-posta tanımlayın.",
          },
          { status: 400 },
        );
      }
    ...
    if (passwordChanging) nextHash = await bcrypt.hash(newPass, 10);

    let nextChangeCount = changeCount;
    if (credentialChange && changeCount === 0) nextChangeCount = 1;
